...
SavedProjectHeader(u64 MagicValue, u64 NumCurves, u64 NumImages,
                  ...
                  string SaveFilePath)
{
   ...
   saved_project_header Header = SavedProjectHeader(SAVED_PROJECT_MAGIC_VALUE,
                                                    Editor.State.NumCurves,
                                                    Editor.State.NumImages,
   ...
   ListIter(Curve, Editor.State.CurvesHead, curve)
   {
      ...
      string SavedCurveData =
         StringMake(Scratch.Arena,
                    cast(char const *)&SavedCurve,
                    sizeof(SavedCurve));
      StringListPush(Scratch.Arena, &SaveData, SavedCurveData);
   ...
   {
      b32 IsSelected = (Editor.State.SelectedEntityType == SelectedEntity_Image &&
                        Image == Editor.State.SelectedEntity.Image);
      ...
      string SavedImageData = StringMake(Scratch.Arena,
                                         cast(char const *)&SavedImage,
                                         sizeof(SavedImage));
      StringListPush(Scratch.Arena, &SaveData, SavedImageData);
      
      StringListPush(Scratch.Arena, &SaveData, ImagePath);
   }
...
LoadProjectFromFile(arena *Arena, pool *CurvePool,
                    pool *ImagePool, arena *DeCasteljauVisualizationArena,
                    ...
                    arena *CurveAnimationArena, string ProjectFilePath)
{
   ...
   if (ReadError == 0)
   {
      ...
      if (Header)
      {
         ...
         {
            editor_state State = EditorStateMake(CurvePool, ImagePool,
                                                 Header->CurveCounter,
                                                 Header->Camera,
            ...
            //- Parse curves
            u64 NumCurves = Header->NumCurves;
            for (u64 CurveIndex = 0;
                 CurveIndex < NumCurves;
                 ++CurveIndex)
            {
               saved_project_curve *SavedCurveState = ExpectTypeInData(saved_project_curve,
                                                                       &Data, &BytesLeft);
               
               if (SavedCurveState)
               {
                  curve Curve = CurveMake(SavedCurveState->CurveName,
                                          SavedCurveState->CurveParams,
                                          SavedCurveState->SelectedControlPointIndex,
                                          SavedCurveState->Position,
                                          SavedCurveState->Rotation);
                  
                  curve *AddCurve = EditorStateAddCurve(&State, Curve);
                  if (SavedCurveState->IsSelected)
                  {
                     EditorStateSelectCurve(&State, AddCurve,
                                            AddCurve->SelectedControlPointIndex);
                  }
                  
                  u64 NumControlPoints = SavedCurveState->NumControlPoints;
               ...
               {
                  Error = StringMakeFormat(Arena,
                                           "project file %s corrupted (unexpected end of data, expected saved curve data)",
                                           ProjectFilePath);
               }
            ...
            if (!Error)
            {
               u64 NumImages = Header->NumImages;
               for (u64 ImageIndex = 0;
                    ImageIndex < NumImages;
                    ++ImageIndex)
               {
                  saved_project_image *SavedImage = ExpectTypeInData(saved_project_image,
                                                                     &Data, &BytesLeft);
                  ...
                  if (SavedImage)
                  {
                     ...
                     if (ImagePathStr)
                     {
                        ...
                        else
                        {
                           ...
                           image *AddedImage = EditorStateAddImage(&State, NewImage);
                           if (SavedImage->IsSelected)
                           {
                              EditorStateSelectImage(&State, AddedImage);
                           }
                        }
                     }
                     else
                  ...
                  {
                     string Warning = StringMakeFormat(Arena,
                                                       "project file %s corrupted (unexpected end of data, expected saved image data)",
                                                       ProjectFilePath);
                     StringListPush(Arena, &Warnings, Warning);
                  }
               }
